  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setSessions(prev => prev.map(s => 
        s.id === currentSession.id ? sessionRes.data : s
      ))
    } catch (error) {
      message.error('发送失败')
      // 失败时也要恢复输入
                      ...
                      >
                        ...
                        <div className="message-content" style={{ margin: 0 }}>
                          <ReactMarkdown
                            ...
                            components={{
                              p: ({ children }) => <Paragraph style={{ margin: 0, whiteSpace: 'pre-wrap' }}>{children as any}</Paragraph>,
                              ul: ({ children }) => <ul style={{ paddingLeft: 20, margin: '4px 0' }}>{children as any}</ul>,
                              ol: ({ children }) => <ol style={{ paddingLeft: 20, margin: '4px 0' }}>{children as any}</ol>,
                              li: ({ children }) => <li style={{ marginBottom: 2 }}>{children as any}</li>,
                              code: ({ children }) => <code style={{ background: '#f5f5f5', padding: '2px 4px', borderRadius: 3, fontSize: '0.9em' }}>{children as any}</code>,
                              pre: ({ children }) => <pre style={{ background: '#f5f5f5', padding: 8, borderRadius: 4, overflow: 'auto' }}>{children as any}</pre>,
                            }}
                          >
                            {msg.content}
                          </ReactMarkdown>
