#!/usr/bin/env node
/**
 * NewClaw Feishu 长连接（使用 SDK WSClient）- 带连接状态监控
 */
...
async function main() {
  console.log('🚀 NewClaw Feishu 长连接（带连接状态监控）');
  console.log('===========================================');
  console.log('');
  ...
  const wsClient = new Lark.WSClient({
    ...
    loggerLevel: Lark.LoggerLevel.debug, // 使用 debug 级别
  });
  ...
  eventDispatcher.register({
    'im.message.receive_v1': async (data) => {
      try {
        ...
        console.log('   消息:', JSON.stringify(data).substring(0, 200));
  ...
  setInterval(() => {
    ...
    console.log(`📊 连接状态: ${new Date().toISOString()}`);
    console.log(`   最后连接时间: ${new Date(reconnectInfo.lastConnectTime).toISOString()}`);
    console.log(`   下次连接时间: ${new Date(reconnectInfo.nextConnectTime).toISOString()}`);
  }, 30000);
